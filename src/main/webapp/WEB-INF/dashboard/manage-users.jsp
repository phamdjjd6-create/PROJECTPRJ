...
<div class="main">
    ...
    <div class="content">
        ...
        <div class="stats-row">
            <div class="stat-pill gold"><div><div class="num">${totalCustomers}</div><div class="lbl">Khách Hàng</div></div></div>
            <div class="stat-pill blue"><div><div class="num">${totalEmployees}</div><div class="lbl">Nhân Viên</div></div></div>
        </div>
        ...
        <div class="tab-panel active" id="panel-customers">
            ...
            <div class="table-wrap">
                <c:choose>
                        ...
                        <table id="cusTable">
                            ...
                            <c:forEach var="c" items="${customers}">
                                <tr>
                                    ...
                                    <td>${c.phoneNumber}</td>
                                    <td>
                                        <span class="badge ${c.typeCustomer=='Diamond'?'badge-diamond':c.typeCustomer=='Gold'?'badge-gold':c.typeCustomer=='Silver'?'badge-silver':'badge-normal'}">
                                            ${c.typeCustomer=='Diamond'?'💎':c.typeCustomer=='Gold'?'🥇':c.typeCustomer=='Silver'?'🥈':'👤'}
                                            ${not empty c.typeCustomer ? c.typeCustomer : 'Normal'}
                                        </span>
                                    </td>
                    ...
                    <c:otherwise><div class="empty"><div style="font-size:36px;margin-bottom:12px">👥</div><p>Chưa có khách hàng</p></div></c:otherwise>
                </c:choose>
            </div>
        </div>
        ...
        <div class="tab-panel" id="panel-employees">
            ...
            <div class="table-wrap">
                <c:choose>
                        ...
                        <table id="empTable">
                            ...
                            <c:forEach var="e" items="${employees}">
                                <tr>
                                    ...
                                    <td><span class="badge ${e.role=='ADMIN'?'badge-admin':'badge-staff'}">${e.role=='ADMIN'?'👑':'👤'} ${e.role}</span></td>
                                    <td><span class="badge ${e.isActive?'badge-active':'badge-locked'}">${e.isActive?'✅ Đang làm':'⛔ Nghỉ'}</span></td>
                                    <c:if test="${canManageEmployee}">
                                    <td>
                                        <div style="display:flex;gap:6px">
                                            <button class="btn-sm btn-edit"
                                                onclick="openEditEmployee('${e.id}','${e.salary}','${e.position}','${e.role}','${e.fullName}','${e.email}','${e.deptId}','${e.isActive}')">
                                                ✏️ Sửa
                                            </button>
                                            <form method="post" action="${pageContext.request.contextPath}/dashboard/action"
                    ...
                    <c:otherwise><div class="empty"><div style="font-size:36px;margin-bottom:12px">👔</div><p>Chưa có nhân viên</p></div></c:otherwise>
                </c:choose>
            </div>
        </div>
        </c:if>
...
<div id="modalEmployee" class="modal-overlay">
    <div class="modal-box">
        <div class="modal-hdr">
            <div><div class="modal-sub" style="color:var(--gold)">Nhân Viên</div><div class="modal-title" id="editEmpName">Sửa Thông Tin</div></div>
...
<div id="modalCustomer" class="modal-overlay">
    <div class="modal-box">
        <div class="modal-hdr">
            <div><div class="modal-sub" style="color:#60a5fa">Khách Hàng</div><div class="modal-title" id="editCusName">Sửa Thông Tin</div></div>
...
<div id="modalAddCustomer" class="modal-overlay">
    <div class="modal-box">
        <div class="modal-hdr">
            <div><div class="modal-sub" style="color:#4ade80">Khách Hàng Mới</div><div class="modal-title">Thêm Khách Hàng</div></div>
...
<div id="modalAddEmployee" class="modal-overlay">
    <div class="modal-box">
        <div class="modal-hdr">
            <div><div class="modal-sub" style="color:#60a5fa">Nhân Viên Mới</div><div class="modal-title">Thêm Nhân Viên</div></div>
...
<div id="modalVoucher" class="modal-overlay">
    <div class="modal-box">
        <div class="modal-hdr">
            <div><div class="modal-sub" style="color:#4ade80">Tặng Ưu Đãi</div><div class="modal-title" id="voucherName">Khách Hàng</div></div>
        ...
        <div id="voucherPicker">
            ...
            <div style="display:flex;justify-content:flex-end;gap:10px">
                ...
                <button id="btnGiveVoucher" class="btn-save" onclick="giveVoucher()"
                        style="background:linear-gradient(135deg,#4ade80,#22c55e);color:#000" disabled>
                    🎁 Tặng Voucher
                </button>
            </div>
        </div>
